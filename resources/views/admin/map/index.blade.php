    ...
    <main>
        ...
        <div class="container mt-n10">
            <div class="card mb-4">
                ...
                {{--@can('map-create')
                    <div class="card-body">
                        <p><a class="btn btn-primary" href="{{ route('admin.map.create') }}">Նոր Քարտեզ</a></p>
                    </div><!--row-->
                @endcan--}}
                <div class="card-body">
                           ...
                           <table class="table table-hover">
                               ...
                               @foreach($map as $item)
                                   <tr>
                                       ...
                                       <td>{{ $item->lang }}</td>
                                       <td>
                                            <button class="btn btn-primary view_iframe" id="{{route('hrefmap', $item->id) }}">Դիտել</button>
                                       </td>
                                       <td class='float-left'>
                                           <div class="btn-group btn-group-toggle float-right">
                                               <a class="btn btn-warning" href="{{route('map-edit', $item->id) }}">Փոփոխել</a>
<!--                                               <a class="btn btn-danger" onclick="return confirm('Համոզվա՞ծ եք որ ցանկանւմ եք ջնջել')" href="{{route('destroy', $item->id)}}">Ջնջել</a>-->
                                           </div>
                                       </td>
                                   </tr>
                               @endforeach
