                                    ...
                                    @if(isset($menus[$localeCode]))
                                     @foreach($menus[$localeCode] as $item)
                                         <tr>
                                             <td>{{ $item->title }}</td>
                                             <td>{{ $item->path }}</td>
                                             <td><button type="submit" class="badge badge-warning">Փոփոխել</button></td>
                                             <td>
                                                 <a href="{{ route('admin.footerBottomMenus.edit', $item->id) }}" class="badge badge-primary">Փոփոխել</a>
                                                 <form method="post" action="{{ route('admin.footerBottomMenus.destroy', $item->id) }}" class="d-inline">
                                                     @method('delete')
                                                     @csrf
