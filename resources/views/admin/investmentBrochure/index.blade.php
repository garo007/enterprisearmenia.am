    ...
    <main>
        ...
        <div class="container mt-n10">
            <div class="card mb-4">
                ...
                @can('tags-create')
                    <div class="card-body">
                        <p><a class="btn btn-primary" href="{{ route('admin.investmentBrochure.create') }}">Ավելացնել</a></p>
                    </div><!--row-->
                @endcan

                <div class="card-header">{{ $title }}</div>
                <div class="card-body">
                    <form method="post" action="{{ route('admin.investmentBrochure.storePageSettings') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    @if(settings()->group('investmentBrochure_Page_Image')->has('page_img'))
                                        <img  style="max-width: 118px" src="{{ showForAdminPageImage(settings()->group('investmentBrochure_Page_Image')->get('page_img')) }}" alt="">
                                    @else
                                        {{--no-image.jpg--}}
                            ...
                            @if(settings()->group('investmentBrochure_Page_Image')->get('page_img'))
                                <div class="col-md-2">
                            ...
                            <div class="tab-pane fade show {{ (App::getLocale() ==  $localeCode) ? 'active' : ''}}" id="tab_{{$localeCode}}" role="{{$localeCode}}" aria-labelledby="pills-home-tab">
                                <table class="table table-hover">
                                    ...
                                    @foreach($posts as $item)
                                        <tr>
                                            <td>{{ $item->id }}</td>
                                            <td><a href="{{ route('investmentBrochure.index') }}"  class="badge badge-primary">Տեսնել</a></td>
                                            <td>
                                                <a href="{{ route('admin.investmentBrochure.edit', $item->id) }}" class="badge badge-primary">Փոփոխել</a>
                                                <form method="post" action="{{ route('admin.investmentBrochure.destroy', $item->id) }}" class="d-inline">
                                                    @method('delete')
                                                    @csrf
