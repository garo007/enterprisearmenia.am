    ...
    <main>
        ...
        <div class="container mt-n10">
            <div class="card mb-4">
                ...
                @can('posts-create')
                    <div class="card-body">
                        <p><a class="btn btn-primary" href="{{ route('admin.categories.create') }}">Ավելացնել միջոցառում</a></p>
                    </div><!--row-->
                @endcan
                                    ...
                                    @foreach($posts as $item)
                                      @if($item->hasTranslation('name', $localeCode))
                                          <tr>
                                              ...
                                              <td><a href="{{ route('categoryPosts', $item->slug) }}"  class="badge badge-primary">Կայքում</a></td>
                                              <td>
                                                  <a href="{{ route('admin.categories.edit', $item->id) }}" class="badge badge-primary">Փոփոխել</a>
                                                  <form method="post" action="{{ route('admin.categories.destroy', $item->id) }}" class="d-inline">
                                                      @method('delete')
                                                      @csrf
