    ...
    <main>
        ...
        <div class="container mt-n10">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('admin.pages.pageinnovations.store') }}" enctype="multipart/form-data">
                        @csrf
                            ...
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                <div class="tab-pane fade show {{ (App::getLocale() ==  $localeCode) ? 'active' : ''}}" id="tab_{{$localeCode}}" role="{{$localeCode}}" aria-labelledby="pills-home-tab">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="chart_text_middle_left_id[{{$localeCode}}]" class="form-control  select2">
                                            <option value="">Ընտրել</option>
                                            @foreach($charts[$localeCode] as $item)
                                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                            @endforeach
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="chart_text_middle_right_id[{{$localeCode}}]" class="form-control  select2">
                                            <option value="">Ընտրել</option>
                                            @foreach($charts[$localeCode] as $item)
                                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                                            @endforeach
                                    ...
                                    <x-forms.ckeditor name="text_middle_left[{{$localeCode}}]" value=""/>

                                    <label for="my-editor-text_middle_right-{{$localeCode}}">Մեջտեղ աջ տեքստ</label>
                                    <x-forms.ckeditor name="text_middle_right[{{$localeCode}}]" value=""/>
