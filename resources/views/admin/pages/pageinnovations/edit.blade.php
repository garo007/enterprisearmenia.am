    ...
    <main>
        ...
        <div class="container mt-n10">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('admin.pages.pageinnovations.update', $post->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="page_img_mini">Էջի մեծ նկար</label>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="page_img_middle">Էջի փոքր նկար</label>
                        ...
                        <div class="tab-content" id="pills-tabContent-556">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                <div class="tab-pane fade show {{ (App::getLocale() ==  $localeCode) ? 'active' : ''}}" id="tab_{{$localeCode}}" role="{{$localeCode}}" aria-labelledby="pills-home-tab">
                                    ...
                                    <div class="form-group">
                                        <label >Գրաֆիկ մեջտեղի ձախ տեքստի տակ</label>
                                        <select style="width: 500px" name="chart_text_middle_left_id[{{$localeCode}}]" class="form-control select2">
                                            <option value="">Ընտրել</option>
                                            @foreach($charts[$localeCode] as $item)
                                                @if($post->getTranslation('chart_text_middle_left_id',$localeCode) == $item->id)
                                                    <option selected value="{{ $item->id }}">{{ $item->name }}</option>
                                                @else
                                                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                                                @endif
                                    ...
                                    <div class="form-group">
                                        <label >Գրաֆիկ աջ տեքստի տակ</label>
                                        <select style="width: 500px" name="chart_text_middle_right_id[{{$localeCode}}]" class="form-control select2">
                                            <option value="">Ընտրել</option>
                                            @foreach($charts[$localeCode] as $item)
                                                @if($post->getTranslation('chart_text_middle_right_id',$localeCode) == $item->id)
                                                    <option selected value="{{ $item->id }}">{{ $item->name }}</option>
                                                @else
                                                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                                                @endif
                                    ...
                                    <label for="my-editor-text_middle_left-{{$localeCode}}">Մեջտեղ ձախ տեքստ</label>
                                    <x-forms.ckeditor name="text_middle_left[{{$localeCode}}]" value="{!! $post->getTranslation('text_middle_left', $localeCode) !!}"/>


                                    <label for="my-editor-text_middle_right-{{$localeCode}}">Մեջտեղ աջ տեքստ</label>
                                    <x-forms.ckeditor name="text_middle_right[{{$localeCode}}]" value="{!! $post->getTranslation('text_middle_right', $localeCode) !!}"/>
